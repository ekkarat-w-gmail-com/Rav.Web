import * as keys from './keys';

export default {
  [keys.BUY_BUTTON_CHOOSE_SIZE]: 'Select a size',
  [keys.BUY_BUTTON_OUT_OF_STOCK]: '{title} is out of stock',
  [keys.BUY_BUTTON_SELECT]: 'Buy',
  [keys.PRODUCT_META_SKU]: 'SKU',
  [keys.CHECKOUT_CART_TITLE]: 'Cart',
  [keys.CHECKOUT_FREE_SHIPPING]: 'You have free shipping',
  [keys.CHECKOUT_DISCOUNT]: 'Discount',
  [keys.CHECKOUT_TAX]: 'Tax',
  [keys.CHECKOUT_SHIPPING]: 'Shipping',
  [keys.CHECKOUT_SUBTOTAL]: 'Subtotal',
  [keys.CHECKOUT_TOTAL]: 'Total',
  [keys.CHECKOUT_GO_TO_CHECKOUT]: 'Checkout',
  [keys.CHECKOUT_KEEP_SHOPPING]: 'Keep shopping',
  [keys.CART_ITEM_REMOVE]: 'Remove',
  [keys.PRODUCT_ACCORDION_DESCRIPTION]: 'Description',
  [keys.PRODUCT_ACCORDION_SPECIFICATION]: 'Specification',
  [keys.PRODUCT_ACCORDION_CARE_INSTRUCTIONS]: 'Care Instructions',
}
